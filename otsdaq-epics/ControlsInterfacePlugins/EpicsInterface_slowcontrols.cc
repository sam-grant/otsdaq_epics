...
EpicsInterface::EpicsInterface(
    const std::string &pluginType, const std::string &interfaceUID,
    const ConfigurationTree &theXDAQContextConfigTree,
    const std::string &controlsConfigurationPath)
    : SlowControlsVInterface(pluginType, interfaceUID, theXDAQContextConfigTree,
                             controlsConfigurationPath) {
...
std::string EpicsInterface::getList(const std::string &format) {
  ...
  if (format == "JSON") {
    __GEN_COUT__ << "Getting list in JSON format! There are "
                 << mapOfPVInfo_.size() << " pv's.";
    // pvList = "{\"PVList\" : [";
    pvList = "[";
    for (auto it = mapOfPVInfo_.begin(); it != mapOfPVInfo_.end(); it++) {
      if (dbconnStatus_ == 1) {
        ...
        if (PQresultStatus(res) == PGRES_TUPLES_OK) {
          ...
          __GEN_COUT__ << "getList() \"sample rate\" SELECT result: "
                       << it->first << ":" << refreshRate
                       << " (smpl_mode_id = " << smplMode << ")" << __E__;
        } else {
...
void EpicsInterface::subscribeJSON(const std::string &JSONNameString) {
  ...
  if (pvList.find(JSON) != std::string::npos) {
    pvList =
        pvList.substr(pvList.find(JSON) + JSON.length(), std::string::npos);
    do {
...
void EpicsInterface::staticChannelCallbackHandler(
    struct connection_handler_args cha) {
  __COUT__ << "webClientChannelCallbackHandler" << __E__;

  ((PVHandlerParameters *)ca_puser(cha.chid))
      ->webClient->channelCallbackHandler(cha);
  return;
}

void EpicsInterface::channelCallbackHandler(
    struct connection_handler_args &cha) {
...
void EpicsInterface::loadListOfPVs() {
  ...
  for (i = 2; i < files.size(); i++) {
    ...
    if (!infile.is_open()) {
      __GEN_SS__ << "Failed to open PV list file: '" << pv_list_file << "'"
                 << __E__;
      __GEN_SS_THROW__;
    }
...
void EpicsInterface::getControlValues(const std::string &pvName) {
  if (true) {
    __GEN_COUT__ << "EpicsInterface::getControlValues(" << pvName << ")"
                 << __E__;
  }
  if (!checkIfPVExists(pvName)) {
    __GEN_COUT__ << pvName << " doesn't exist!" << __E__;
  ...
  SEVCHK(ca_array_get_callback(
             // DBR_CTRL_CHAR,
             DBR_CTRL_DOUBLE, 0, mapOfPVInfo_.find(pvName)->second->channelID,
             eventCallback, this),
...
void EpicsInterface::createChannel(const std::string &pvName) {
  ...
  SEVCHK(ca_create_channel(pvName.c_str(), staticChannelCallbackHandler,
                           mapOfPVInfo_.find(pvName)->second->parameterPtr, 0,
  ...
  SEVCHK(
      ca_replace_access_rights_event(
          mapOfPVInfo_.find(pvName)->second->channelID, accessRightsCallback),
...
void EpicsInterface::accessRightsCallback(
...
void EpicsInterface::printChidInfo(chid chid, const std::string &message) {
  ...
  __COUT__ << "read(" << ca_read_access(chid) << ") write("
           << ca_write_access(chid) << ") state(" << ca_state(chid) << ")"
           << __E__;
}
...
void EpicsInterface::subscribeToChannel(const std::string &pvName,
  ...
  SEVCHK(ca_create_subscription(
             dbf_type_to_DBR(mapOfPVInfo_.find(pvName)->second->channelType), 1,
             mapOfPVInfo_.find(pvName)->second->channelID,
             DBE_VALUE | DBE_ALARM | DBE_PROPERTY, eventCallback, this,
             &(mapOfPVInfo_.find(pvName)->second->eventID)),
         "EpicsInterface::subscribeToChannel() : ca_create_subscription "
         "dbf_type_to_DBR");

  SEVCHK(ca_create_subscription(
             DBR_STS_DOUBLE, 1, mapOfPVInfo_.find(pvName)->second->channelID,
             DBE_VALUE | DBE_ALARM | DBE_PROPERTY, eventCallback, this,
             &(mapOfPVInfo_.find(pvName)->second->eventID)),
         "EpicsInterface::subscribeToChannel() : ca_create_subscription "
         "DBR_STS_DOUBLE");
  ...
  SEVCHK(ca_create_subscription(
             DBR_CTRL_DOUBLE, 1, mapOfPVInfo_.find(pvName)->second->channelID,
             DBE_VALUE | DBE_ALARM | DBE_PROPERTY, eventCallback, this,
             &(mapOfPVInfo_.find(pvName)->second->eventID)),
         "EpicsInterface::subscribeToChannel() : ca_create_subscription");
...
void EpicsInterface::cancelSubscriptionToChannel(const std::string &pvName) {
  if (mapOfPVInfo_.find(pvName)->second != NULL)
    if (mapOfPVInfo_.find(pvName)->second->eventID != NULL) {
      status_ =
          ca_clear_subscription(mapOfPVInfo_.find(pvName)->second->eventID);
      SEVCHK(status_, "EpicsInterface::cancelSubscriptionToChannel() : "
...
void EpicsInterface::writePVControlValueToRecord(
  ...
  if (DEBUG) {
    ...
    __GEN_COUT__ << "upper disp limit: " << (int)(pdata->upper_disp_limit)
                 << __E__;
    __GEN_COUT__ << "lower disp limit: " << pdata->lower_disp_limit << __E__;
    __GEN_COUT__ << "upper alarm limit: " << pdata->upper_alarm_limit << __E__;
    __GEN_COUT__ << "upper warning limit: " << pdata->upper_warning_limit
                 << __E__;
    __GEN_COUT__ << "lower warning limit: " << pdata->lower_warning_limit
                 << __E__;
...
void EpicsInterface::writePVValueToRecord(const std::string &pvName,
  ...
  if (pvInfo->mostRecentBufferIndex != pvInfo->dataCache.size() - 1 &&
    ...
    if (pvInfo->dataCache[pvInfo->mostRecentBufferIndex].first ==
        currentRecord.first) {
...
void EpicsInterface::readPVRecord(const std::string &pvName) {
  status_ = ca_array_get_callback(
      ...
      mapOfPVInfo_.find(pvName)->second->channelID, eventCallback, this);
...
void EpicsInterface::debugConsole(const std::string &pvName) {
  ...
  for (unsigned int it = 0;
       it < mapOfPVInfo_.find(pvName)->second->dataCache.size() - 1; it++) {
    ...
    __GEN_COUT__ << "Iteration: " << it << " | "
                 ...
                 << " | "
                 << mapOfPVInfo_.find(pvName)->second->dataCache[it].second
                 << __E__;
    if (it == mapOfPVInfo_.find(pvName)->second->mostRecentBufferIndex) {
      __GEN_COUT__
          << "-----------------------------------------------------------"
  ...
  __GEN_COUT__ << "Status:     "
               ...
               << " | "
               << mapOfPVInfo_.find(pvName)->second->alerts.front().status
               << __E__;
  __GEN_COUT__ << "Severity:   "
               << " | " << mapOfPVInfo_.find(pvName)->second->alerts.size()
               << " | "
               << mapOfPVInfo_.find(pvName)->second->alerts.front().severity
               << __E__;
  __GEN_COUT__
      << "==============================================================="
         "==============="
...
EpicsInterface::getCurrentValue(const std::string &pvName) {
  ...
  std::array<std::string, 4> currentValues = {"PV Not Found", "NF", "N/a",
                                              "N/a"};
...
EpicsInterface::getSettings(const std::string &pvName) {
  ...
  if (mapOfPVInfo_.find(pvName) != mapOfPVInfo_.end()) {
    ...
    //    "DC'd", "DC'd", "DC'd", "DC'd", "DC'd", "DC'd", "DC'd", "DC'd",
    //    "DC'd"};

    std::array<std::string, 9> s = {
        units,           upperDisplayLimit, lowerDisplayLimit,
        upperAlarmLimit, upperWarningLimit, lowerWarningLimit,
  ...
  std::array<std::string, 9> s = {"DC'd", "DC'd", "DC'd", "DC'd", "DC'd",
                                  "DC'd", "DC'd", "DC'd", "DC'd"};
  return s;
}
...
void EpicsInterface::dbSystemLogin() {
  ...
  dbconn = PQconnectdb("dbname=dcs_archive host=mu2edaq12 port=5432 "
...
EpicsInterface::getChannelHistory(const std::string &pvName) {
...
EpicsInterface::checkAlarm(const std::string &pvName,
  ...
  if (severity == EPICS_NO_ALARM ||
      (ignoreMinor && severity == EPICS_MINOR_ALARM))
...
std::vector<std::vector<std::string>> EpicsInterface::checkAlarms() {
  ...
  for (auto pvPair : mapOfPVInfo_) {
    ...
    std::string &severity = valueArray[3];
    if (severity != EPICS_NO_ALARM) {
      channelAlarms.push_back(std::vector<std::string>(
          {pvPair.first, time, value, status, severity}));
...
//========================================================================================================================
// Check Alarms from Epics
void EpicsInterface::handleAlarmsForFSM(
  ...
  if (!linkToAlarmsToMonitor.isDisconnected()) {
    ...
    for (const auto &alarmToMonitor : alarmsToMonitor) {
      std::vector<std::string> alarmReturn =
          checkAlarm(alarmToMonitor.second.getNode("AlarmChannelName")
                         .getValue<std::string>(),
                     alarmToMonitor.second.getNode("IgnoreMinorSeverity")
                         .getValue<bool>());

      if (alarmReturn.size()) {
        ss << "Found alarm for channel '" << alarmReturn[0] << "' = {"
           << "time=" << alarmReturn[1] << ", value=" << alarmReturn[2]
           << ", status=" << alarmReturn[3] << ", severity=" << alarmReturn[4]
           << "}!" << __E__;
        ++foundCount;
      }
    }
